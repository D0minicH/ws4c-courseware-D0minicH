package ch.fhnw.uieng.module04.mountainapp.view.util;
...
public class RoundImageViewSkin extends SkinBase<RoundImageView> {
        ...
        if (imageView.getImage() != null) {
            ...
            if (currentImage.getHeight() > currentImage.getWidth()) {
                ...
                imageView.setViewport(
                        new Rectangle2D(0, (currentImage.getHeight() - contentHeight * factor) / 2, currentImage.getWidth(), currentImage.getHeight()));
            } else {
                imageView.setFitHeight(contentHeight);
                imageView.setFitWidth(-1);
                double factor = currentImage.getHeight() / contentHeight;
                imageView.setViewport(
                        new Rectangle2D((currentImage.getWidth() - contentWidth * factor) / 2, 0, currentImage.getWidth(), currentImage.getHeight()));
            }
    ...
    private double computeSize() {
        return getSkinnable().getDefaultSize();
    }
