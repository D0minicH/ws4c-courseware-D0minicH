...
public class PagingList<PM extends PMBase<DTO>, DTO> extends ObservableListBase<PM> {
    ...
    private TaskBatcher taskBatcher = new TaskBatcher(Duration.ofMillis(80));
    ...
    public PagingList(PagingService<DTO> service, Function<Integer, PM> pmFactory) {
        ...
        firstIndexShown.addListener((observable, oldValue, newValue) -> taskBatcher.batch(() -> loadPage(newValue.intValue())));
    ...
    private void loadPage(int index) {
        ...
        if (index >= actualFirstInCache && index + getVisibleRows() <= actualFirstInCache + 2 * pageSize) {
            return;
        }
        int firstIndex = Math.max(0, index - pageSize);
