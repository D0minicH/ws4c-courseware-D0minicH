...
import ch.fhnw.uieng.module05.mountainform.presentationmodel.MountainDetailPM;
...
import ch.fhnw.uieng.module05.mountainform.view.util.rectangularimageview.RectangularImageView;

/**
 * @author Dieter Holz
 */
class Summary extends GridPane implements ViewMixin {
    ...
    Summary(Switzerland rootSwitzerland) {
    ...
    public void setupValueChangedListeners() {
        rootSwitzerland.getCurrentPM().cantonsProperty().addListener((observable, oldValue, newValue) -> {
            cantonsBox.getChildren().clear(); //toDo: muss ins Switzerland
            if (newValue != null) {
    ...
    public void setupBindings() {
        MountainDetailPM mountain = rootSwitzerland.getCurrentPM();

        nameLabel.textProperty()   .bind(mountain.nameProperty());
        heightLabel.textProperty() .bind(mountain.heightProperty().asString("%.2f m"));
        rangeLabel.textProperty()  .bind(mountain.rangeProperty());
        captionLabel.textProperty().bind(mountain.captionProperty());
        picture.imageURLProperty() .bind(mountain.imageURLProperty());
    }

    private ImageView createImageView(String canton, String cantonName) {
        ImageView imageView = new ImageView(new Image(Summary.class.getResourceAsStream("/wappen_klein/" + canton + ".png")));
